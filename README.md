...
- Automated Deployments (Argo CD): My GitOps engine. It watches this repository and automatically updates the apps in the cluster whenever I push a change.
...
- Logs & Monitoring (Grafana Alloy + LGTM Stack): Grafana Alloy collects all the data from the cluster. It maps everything into Loki (logs), Tempo (traces), and Mimir (metrics) so I can visually track exactly what the apps are doing on Grafana dashboards.
